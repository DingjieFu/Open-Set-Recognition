...
class MyDataset(Dataset):
    def __init__(self, dataset, input_size, known_percent=0):
        ...
        trainData_file = splitRoot + f'/{self.dataset}_train_data.npy'
        testData_file = splitRoot + f'/{self.dataset}_test_data.npy'
        if os.path.exists(trainData_file) and os.path.exists(testData_file):
            print(" ==> Load existing data")
            self.trainData = np.load(trainData_file)
            self.testData = np.load(testData_file)
        else:
            print(" ==> Extract data")
    ...
    def path2data(self):
        ...
        prefix = os.path.dirname(__file__) + "/splitFiles/"
        np.save(prefix + f'{self.dataset}_train_data.npy', train_data)
        np.save(prefix + f'{self.dataset}_test_data.npy', test_data)
