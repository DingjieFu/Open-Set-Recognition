...
imageRoot = "/data/fudingjie/FGVC/CUB/CUB_200_2011"
# get image paths
with open(os.path.join(imageRoot, "images.txt")) as f:
    lines = f.readlines()
    paths_suffix = [line.strip().split()[-1] for line in lines]
    path_all = [imageRoot + "/images/" + path_suffix for path_suffix in paths_suffix]

# get corresponding labels
with open(os.path.join(imageRoot, "image_class_labels.txt")) as f:
    lines = f.readlines()
    label_all = np.array([int(line.strip().split()[-1]) for line in lines])

# get classnames
with open(os.path.join(imageRoot, "classes.txt")) as f:
    lines = f.readlines()
    class_names = [line.strip().split()[-1] for line in lines]

# split data
with open(os.path.join(imageRoot, "train_test_split.txt")) as f:
    lines = f.readlines()
